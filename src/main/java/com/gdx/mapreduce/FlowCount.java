...
public class FlowCount {
	...
	static class FlowCountReducer extends Reducer<Text, FlowBean, Text, FlowBean>{
		private static TreeMap<FlowBean,Text> treeMap = new TreeMap<FlowBean,Text>(Collections.reverseOrder());
		...
		protected void reduce(Text key, Iterable<FlowBean> values,
			...
			//遍历所有的bean，将其中的上行流量，下行流量累加
			for (FlowBean flowBean : values) {
				sum_upFlow += flowBean.getUpFlow();
				sum_dFlow += flowBean.getDownFolw();
			}
			
			
			FlowBean bean = new FlowBean(sum_upFlow,sum_dFlow);
			treeMap.put(bean,key);
